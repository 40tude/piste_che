...
<!-- ################################################# -->
A la fin je fais créer une issue sur GitHub
Quand tout est OK je passe la slash command

/speckit.specify
We want to implement the feature described in GitHub issue #3: Add a new Cargo workspace member resort_generator: a single CLI binary that fetches trail and elevation data for a ski resort, merges the results, and writes a timestamped JSON file to data/ folder.