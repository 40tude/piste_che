...
- [ ] T017 [US1] Create `src/server/mod.rs` with `pub mod api`; verify that `#[server]` macros in `api.rs` are sufficient for auto-registration in Leptos 0.7 (no explicit `.register()` calls needed -- that pattern is deprecated since Leptos 0.6)
